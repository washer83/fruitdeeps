...
export class SpecialAttacks {
    ...
    applySpec(specName) {
        console.log(`applySpec called with specName: ${specName}`);
        switch (specName) {
            case "Ninja star":
                return this.ninjaSpec();
            case "Slice and Dice":
                return this.dclaws();
            case "Puncture":
                return this.dds();
    ...
    ninjaSpec() {
        ...
        const dpsRoll = this.calcs.playerRoll;
    
        let hitStore = new HitFreqStore();
        hitStore.store([0], 1 - acc);  // Store miss probability
    
        for (let hit1 = 0; hit1 <= max; hit1++) {
            let dmg1 = hit1 === 0 ? 1 : hit1;
            hitStore.store([dmg1], acc / (max + 1));
        }
    
        let combinedHitStore = new HitFreqStore();
        let hitFreqs = hitStore.getFreqs();
    
        for (let hit1 of hitFreqs) {
            for (let hit2 of hitFreqs) {
                let dmg2 = hit2.dmg[0];
                combinedHitStore.store([hit1.dmg[0], dmg2], hit1.p * hit2.p);
            }
        }
        ...
        eDefReduction1 = this.state.monster.stats.mage * 0.1 * acc1;
        defAfterHit1 = Math.ceil(def - eDefReduction1);
    
        let npcRoll2 = accCalc.generalFormula(defAfterHit1 + 9, this.state.monster.stats.drange);
        let acc2 = accCalc.compareRolls(dpsRoll, npcRoll2);
        eDefReduction2 = this.state.monster.stats.mage * 0.1 * acc2;
    
        eDefReduction = Math.trunc(eDefReduction1 + eDefReduction2);
    
        console.log(`eDefReduction1: ${eDefReduction1}, eDefReduction2: ${eDefReduction2}, Total eDefReduction: ${eDefReduction}`);
    
        dps.hitStore = combinedHitStore;
        dps.eDefReduction = Math.floor(eDefReduction) // Ensure existing value is incremented
        dps.maxList = [max];
        dps.maxHit = max;
        return dps;
    }
